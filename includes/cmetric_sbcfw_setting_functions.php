...
class WP_Class_Cmetric_Sbcfw_Setting  {
                            ...
                            if("free_shipping" === $methodvalue->id)    
                            {
                                ...
                                switch ( $methodvalue->requires ) {
                                  ...
                                  if($is_available){
                                        ...
                                        $newrate = (float) $this->get_shipping_cost_by_instance_id($rates, $ship_id);
    ...
    public function get_shipping_cost_by_instance_id($shipping_rates, $instance_id_to_find) {
      foreach ($shipping_rates as $rate) {
          if ($rate->get_instance_id() == $instance_id_to_find) {
              return $rate->get_cost(); // This returns a string, so it's fine
          }
      }
      return '0'; // Return a default value like '0' instead of null to avoid errors
    }
}
endif;
?>